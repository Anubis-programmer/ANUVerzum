...
2. **Remove stale attributes.** Props present in `prevProps` but absent from `nextProps` are removed via the shared `removeDomProperty` helper (see below).

   **Symmetric removal — `removeDomProperty`.** Removal mirrors the set decision in pass 3 rather than blindly calling `removeAttribute`. A prop that was *written as a property* (`el[name] = value`) is *cleared as a property* (`el[name] = undefined`), because `removeAttribute` is a no-op for a value that never became an attribute — e.g. `inert`, set via `el.inert = 'true'`, would otherwise keep its old value when toggled off. The helper applies the same branch order as the set path: SVG → `removeAttribute` (`className`→`class`); `className`→`removeAttribute('class')`; `htmlFor`→`removeAttribute('for')`; hyphenated / `role` / `ATTRIBUTE_ONLY_PROPS` → `removeAttribute`; camelCase HTML names on element nodes → `removeAttribute` (lowercased / `HTML_ATTRIBUTE_NAME_MAP`); everything else → `el[name] = undefined`. It is used by both this stale-key pass and the nullish-value short-circuit in pass 3.

3. **Set new / changed attributes.** A new/changed prop whose value is `undefined` or `null` is treated as a **removal** — routed through `removeDomProperty` (above), so the property/attribute decision matches how the prop was originally written rather than always coercing to `removeAttribute`. This mirrors the initial-render behaviour, where an `undefined` prop is filtered out by the `prev[key] !== next[key]` diff and never written, so `attr={cond ? value : undefined}` cleanly drops the prop when the condition flips — including property-path props like `inert`. Otherwise, for SVG elements, all attributes are set via `setAttribute()`. For HTML elements:
   ...
   - **boolean values with a reflecting IDL property** (`typeof value === 'boolean' && name in el` — e.g. `readOnly={false}`, `contentEditable={false}`, `formNoValidate`) are set as an IDL property (`el[name] = value`), *before* the camelCase `setAttribute()` branch below. This is required because `setAttribute('readonly', false)` stringifies to `readonly="false"`, and a boolean attribute is truthy by mere presence — so `readOnly={false}` would make the field read-only (`el.readOnly === true`), the opposite of intent. The property path makes `el.readOnly = false` omit the attribute (matching the all-lowercase `disabled`, which already took the property path) and `el.readOnly = true` set it. The `name in el` guard restricts this to names that *are* a working IDL property: `spellCheck`/`autoFocus` (whose IDL is the lowercase `spellcheck`/`autofocus`) fail it and stay on `setAttribute()`, so the enumerated `spellCheck={false}` still reflects `spellcheck="false"` instead of vanishing into a dead `el.spellCheck` expando. String/number camelCase props (`inputMode`, `autoComplete='email'`) are non-boolean and also fall through;
